...
const client = new ImageAnnotatorClient({
    keyFilename: '/home/ubuntu/namuthon/server_nodejs_templete/valued-rigging-409512-fa70fd80beca.json', // 실제 키 파일 경로로 변경해야 합니다.
});
...
exports.postBlood = async function (req, res) {
    ...
    // 함수 사용 예시
    let imagePath = '/home/ubuntu/namuthon/server_nodejs_templete/upload/test.png'; // 이미지 파일 경로로 변경해야 합니다.
    detectTextAndExtractInfo(imagePath).then(info => {
        console.log(info);
    }).catch(error => {
        console.error(error);
    });
...
async function detectTextAndExtractInfo(imagePath) {
    try {
        ...
        // 데이터베이스에 저장
        const query = 'INSERT INTO wallet(w_type,w_date,w_hos,w_num,u_id) VALUES (?,?,?,?,20)';
